...
namespace Caro_game.ViewModels
{
    public partial class MainViewModel
    {
        ...
        private void ApplyGameState(GameState state)
        {
            ...
            var targetLevel = string.IsNullOrWhiteSpace(state.AILevel) ? "Dễ" : state.AILevel!;
            SelectedAILevel = targetLevel;

            bool professionalLevelRestored = state.IsAIEnabled && targetLevel == "Chuyên nghiệp";
            var boardAILevel = professionalLevelRestored ? "Khó" : targetLevel;

            var board = new BoardViewModel(state.Rows, state.Columns, state.FirstPlayer ?? "X", boardAILevel)
            {
                IsAIEnabled = professionalLevelRestored ? false : state.IsAIEnabled
            };
